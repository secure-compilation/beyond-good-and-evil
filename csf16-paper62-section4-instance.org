...
* Compiler (changes in partial programs not propagated yet)
...
* Proofs using trace semantics (changes in trace notation & partial programs not propagated yet)
...
YJ: Yes. Actually they are exactly the same when we unfold
⁻¹. Probably shouldn't keep both, but things will be clearer with the
new notation.
