...
Since C'∉ dom(Eₚ) and C, we have that C'≠C.
Hence
  C'.P' ∈ ψ[C].import.
...
applies. In this case, we can apply action decomposition to this
reduction and get that Pθ reduces with either an internal action or an
external action that isn't termination. But no such reduction is
possible, from what we've learned by inverting T_Exit!.
...
hence we can choose
  ...
  Aθ' = Aθ
  t = ε
and trivially prove left by applying rule T_Refl.
...
We proceed by case on this disjunction. The proof is similar in both
cases.
